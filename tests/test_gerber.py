...
def test_pth_preferred_for_via_alias(tmp_path):
    """PTH remains the preferred source for via.svg when present."""
    pth = tmp_path / 'PTH.svg'
    drill = tmp_path / 'drill.svg'
    ...
    assert (tmp_path / 'via.svg').read_bytes() == b'pth'
    assert not (tmp_path / 'via.png').exists()
